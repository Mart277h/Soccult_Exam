...
Now trying to find a gamma distribution that fits better with the confidence intervals 

```{r}
#Did not succeed, trying all kinds of different things. We are going with normal distributions.
mean <- 0.23
...
x <- seq(0, 100, by= .1)
y <- rgamma(100, shape = alpha, scale = lambda)
plot(y)
df <- data.frame(y)
ggplot(df, aes(y))+ 
